...
\bigskip
If we draw any of {\LARGE\tong{5}\suo{2}\suo{5}} first, we discard {\LARGE\wan{8}} to aim for {\jap sanshoku} of 567. 
\bp
...
\bigskip
It is called ``golden'' because the hand is 1-away from ready for \linebreak {\jap sanshoku} \emph{and} 1-away from ready for {\jap ittsu} (Pure Straight), two of the most popular two-{\jap han yaku} in riichi mahjong. 
...
\bigskip
If we draw {\LARGE\tong{6}} or {\LARGE\suo{4}}, the hand becomes ready for {\jap tanyao + pinfu + sanshoku + iipeiko} (Pure Double Chow). Drawing {\LARGE\suo{7}} also gets us {\jap tanyao + pinfu + sanshoku}, and drawing any of {\LARGE\tong{2} \tong{3} \tong{4} \tong{5}} gets us at least {\jap tanyao + pinfu}, and possibly {\jap iipeiko} as well. 
...
The key to getting {\jap ittsu} is to pay attention to {\bf two non-overlapping runs} in a given suit. For example, suppose a hand has two non- \linebreak overlapping runs such as the following. 
...
\bigskip
Although we see a remote possibility of {\jap ittsu} in {\jap souzu} (bamboos), pursuing it requires we fill in three closed-wait protoruns in {\jap souzu}: \linebreak {\LARGE\suo{1}\suo{3}}, {\LARGE\suo{4}\suo{6}}, and {\LARGE\suo{7}\suo{9}}.
...
\bigskip
Keeping {\LARGE\xi} is safer, but keeping {\LARGE\wan{5}} leaves the possibilities of getting {\jap ittsu} and having {\jap dora}, possibly at the same time. In this case, we would rather discard {\LARGE\xi}. 
...
\bigskip
The hand has a potential to have {\jap pinfu}, so we should not discard any of {\LARGE\wan{1}\tong{9}\xi} at this point. All of these three tiles may appear useless, but they can be the head of a {\jap pinfu} hand when any of them grows into a pair. On the other hand, the value tile {\LARGE\fa} cannot be the head of {\jap pinfu}. We should discard {\LARGE\fa} in this case. 
...
\ep
\vspace{-5pt}Would you go for {\jap honitsu}? \vspace{-5pt}
\end{itembox}
%\noindent
...
\bigskip
In addition, the tiles that are made redundant in the hand if we \linebreak choose {\jap honitsu} are an isolated {\LARGE\tong{8}} and a closed-wait protorun {\LARGE\suo{2}\suo{4}}. Keeping these tiles would not make this hand particularly more efficient anyway, so we can go for {\jap honitsu} without much hesitation. 
...
\bigskip
This hand is worth 5200 points without {\jap honitsu} (Seat Wind + Green Dragon + red five), so you should discard {\LARGE\wan{7}} to maintain a side \& side 1-away status. If {\LARGE\nan} were not your seat wind, you should go for {\jap honitsu}.
...
\bigskip
You should pursue {\jap honitsu}, so {\LARGE\tong{7}\suo{3}\suo{5}} are your discard candidates. You will discard all three of them eventually, but you should discard them in a way that looks less obvious that you are collecting tiles in {\jap manzu} (cracks). 
...
\ep
\vspace{-5pt}Which tile would you call? \vspace{-5pt}
\end{itembox}

\bigskip
With this hand, do not start melding with a {\jap chii} of {\LARGE\tong{4}} or a {\jap pon} of {\LARGE\bei} if you are playing without red fives; you may end up with a very cheap (2000 points) {\jap honitsu}-only hand. 
...
\bigskip
Calling {\jap chii} on {\LARGE\tong{4}} or {\jap pon} on {\LARGE\bei} is still not ideal. The only melding you should do is to call {\jap chii} on {\LARGE\tong{1}} to have the following hand.
...
\bigskip
As we draw {\LARGE\wan{5}}, we now have five pairs in the hand, making it 1-away from ready for {\jap chiitoitsu}. However, the hand is also 2-away from ready if we interpret this hand as a standard hand. 
