...
\bigskip This hand has no {\jap yaku} or {\jap dora}, but the wait is good. You can do insta-riichi with a riichi-only hand as long as the hand has a good wait. 
...
\bigskip You should do insta-riichi even when waiting for {\jap dora}. 
...
\bigskip When you wonder whether or not you should riichi in a given situation, choose riichi. You will be correct most of the time. 
...
\bigskip Here is a simple rule of thumb: when the \emph{minimum} (guaranteed) hand value is below 5200 (when won by {\jap ron}), you should value scores over wait. When the \emph{minimum} hand value is 5200 or above, you should value wait over scores. 
...
\bigskip This hand has one {\jap dora}, but the minimum hand value is still below 5200 (riichi + {\jap dora} = 2600). Therefore, again, you should value scores over wait. 
...
\bigskip With this hand, calling riichi guarantees 5200 (riichi + White Dragon + one {\jap dora}). Therefore, you should value wait over scores this time. It goes without saying that riichi is better than going {\jap dama}. 
...
\bigskip Calling riichi by discarding {\LARGE \wan{2}} gives you riichi-only with a good wait, whereas calling riichi by discarding {\LARGE \wan{5}} gives you riichi + {\jap sanshoku} with a bad wait. Since riichi-only is short of 5200, you should value scores over wait. 
...
\bigskip Calling riichi by discarding {\LARGE \wan{1}} gives you only riichi + {\jap pinfu} with a good wait, whereas calling riichi by discarding {\LARGE \wan{5}} gives you riichi + {\jap ittsu} with a bad wait. Since riichi + {\jap pinfu} is short of 5200, you should value scores over wait. 
\bigskip
\begin{center}
{\Large $\Rightarrow$ Insta-riichi, discarding  \wan{5}~!}
\end{center}
...
\bigskip Discarding {\LARGE\wan{7}} would give you a {\jap dama mangan} hand with a bad wait. That is not too bad, but it is much better to riichi by discarding {\LARGE\wan{1}} (riichi + {\jap pinfu} + two red fives = 7700 with a very good wait). 
...
\bigskip You should not shy away from riichi with a {\jap chiitoitsu} hand, especially when waiting for {\jap dora}. Since {\jap dora} of {\LARGE \tong{4}} is something your opponents wouldn't lightly discard even if you keep {\jap dama}, the chance of winning this hand by {\jap ron} is not very high anyway. Therefore, you would rather riichi and aim to improve the score. 
...
\bigskip
Riichi criteria for {\jap chiitoitsu} hands waiting for {\jap dora} are really simple. 
...
\bigskip This means that the only two cases where you should go {\jap dama} are (a) when you are a non-dealer \emph{and} you are waiting for 4,5,6, and (b) you have {\jap honitsu} (Half Flush) or {\jap chinitsu} (Full Flush) {\jap chiitoitsu}.\footnote{It is also (theoretically) possible to have  {\jap tsu iso} (All Honors) {\jap chiitoitsu}. Do whatever you want with such a once-in-lifetime hand. I would riichi.} The criteria do not change when your hand already has two {\jap dora} and waits for a non-{\jap dora} tile.
...
\bigskip If you make the hand ready for {\jap chiitoitsu}, you will be waiting for {\LARGE\wan{3}}, a non-4,5,6 tile. riichi is better than {\jap dama} in this case. 
...
\bigskip Waiting for a 6 is not ideal, but having {\jap tanyao} justifies riichi. Aim for 6400 {\jap ron} or {\jap mangan} {\jap tsumo}.
...
\bigskip Likewise, waiting for a 5 is not ideal, but having a red five justifies riichi. 
...
\bigskip Since you can get {\jap mangan ron} or {\jap haneman tsumo} without riichi, you should keep {\jap dama} with this hand (discard {\LARGE \suo{7}}). 
...
\bigskip This hand is worth 7700 when winning on {\LARGE \wan{5}} ({\jap tanyao + pinfu + sanshoku}), but the hand is worth only 2000 if winning on {\LARGE \wan{2}} ({\jap tanyao + pinfu}). You should therefore do insta-riichi by discarding {\LARGE\wan{4}}. 
...
\bigskip Discarding {\LARGE\suo{9}}, we get a confirmed 7700 hand without riichi. I already said above that it is acceptable to keep {\jap dama} when the minimum hand value is 7700. However, would calling riichi be even better? 
...
\i If you draw {\LARGE\suo{8}}, the hand is ready for {\jap pinfu + junchan + \linebreak sanshoku} --- {\jap haneman} without riichi. You may still want to riichi. It would be a shame to win this monster hand on {\LARGE\suo{6}}, but doing so without riichi is the worst. 
\newpage
